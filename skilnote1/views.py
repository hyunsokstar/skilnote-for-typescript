...
from accounts2.models import Profile , HistoryForUpdate
...
def intro_for_skilnote(request):
    return render(request, 'skilnote1/intro_page.html', {
	})
...
class MyShortCutListView2(LoginRequiredMixin,ListView):
    ...
    def get_template_names(self):
        if self.request.is_ajax():
            ...
            return ['skilnote1/myshortcut_list2.html']
        return ['skilnote1/myshortcut_list2.html']
...
def insert_temp_skill_note_for_textarea(request):
    ...
    skilnote1 = TempMyShortCut.objects.create(
        author = request.user,
        title=title,
        type= ty,
        category = ca,
...
def insert_for_guest_book(request):
    ...
    return JsonResponse({
        ...
        'guest_book_title': guest_book.title,
...
def copy_to_me_from_user_id(request):
    ...
    skilnote1_list_for_copy = MyShortCut.objects.filter(Q(author=user_id))
    print("skilnote1_list_for_copy : " , skilnote1_list_for_copy);
    MyShortCut.objects.filter(Q(author=request.user)).delete()

    comment_skilnote1_list_for_copy = CommentForShortCut.objects.filter(Q(author=user_id))
...
def insert_temp_skill_note_using_input_for_backend(request):
    ...
    skilnote1 = TempMyShortCutForBackEnd.objects.create(
        author = request.user,
        title=title,
        type= ty,
        category = ca,
...
def temp_skill_list2(request,page_user):
    ...
    return render(request, 'skilnote1s/TempMyShortCut_list.html', {
        'object_list': object_list,
        'page_user': user
    })
...
def create_new4_textarea(request):
    ...
    skilnote11 = MyShortCut.objects.create(
        author = request.user,
        title="title1",
        type= ty,
        category = ca,
...
def create_new1_input(request):
    ...
    skilnote1 = MyShortCut.objects.create(
        author = request.user,
        title=title,
        type= ty,
        category = ca,
...
class modify_myshortcut_by_summer_note2(UpdateView):
    ...
    def get_success_url(self):
        return reverse('skilnote1:my_shortcut_list2')
...
class user_list_for_login_page(ListView):
    ...
    def get_template_names(self):
        if self.request.is_ajax():
            print("user list ajax 요청 확인")
            return ['skilnote1/_user_list_for_login_page.html']
        return ['skilnote1/user_list_for_loginpage.html']
    ...
    def get_queryset(self):
        ...
        if query != None:
            user_list = User.objects.all().filter(Q(username__contains=query)).order_by('-profile__skill_note_reputation');
            return user_list
        else:
            print("user list 출력 확인 ===========================================================")
            object_list = User.objects.all().filter(profile__public="yes").order_by('-profile__skill_note_reputation');
...
class search_skil_note_for_me(LoginRequiredMixin,ListView):
    ...
    def get_template_names(self):
        return ['skilnote1/search_skil_note_for_me.html']
...
class search_skilnote_by_file_name_for_me(LoginRequiredMixin,ListView):
    ...
    def get_template_names(self):
        return ['skilnote1/search_skil_note_for_file_name_for_me.html']
...
class search_skilnote_by_file_name_for_all(LoginRequiredMixin,ListView):
    ...
    def get_template_names(self):
        return ['skilnote1/search_skil_note_for_file_name_for_all.html']
...
class search_skil_note_for_all(LoginRequiredMixin,ListView):
    ...
    def get_template_names(self):
        return ['skilnote1/search_skil_note_for_all_user.html']
...
class MyShortCutCreateView_input(LoginRequiredMixin,CreateView):
    ...
    def get_success_url(self):
        return reverse('skilnote1:my_shortcut_list')
...
class CreateSkilNoteBySummerNote(LoginRequiredMixin,CreateView):
    ...
    def get_success_url(self):
        category_id = self.request.user.profile.selected_category_id
        return reverse('skilnote1:my_shortcut_list')+'#shortcut_{}'.format(category_id)
...
class createSkilNoteForInsertMode(LoginRequiredMixin,CreateView):
    ...
    def form_valid(self, form):
        ...
        # 이부분에서 accounts2 앱의 HistoryForUpdate 모델 에 데이터를 추가 하려면 어떻게 해야 되나요?
        # ca.name , request.user 등을 이용
        # 문제는 create 함수 호출하면 unique failed 에러가 발생합니다.

        return super().form_valid(form)

    def get_success_url(self):
        category_id = self.request.user.profile.selected_category_id
        return reverse('skilnote1:my_shortcut_list2')+'#shortcut_{}'.format(category_id)
...
class SkilNoteCreateView_summernote_through2(LoginRequiredMixin,CreateView):
    ...
    def get_success_url(self):
        category_id = self.request.user.profile.selected_category_id
        return reverse('skilnote1:my_shortcut_list2')+'#shortcut_{}'.format(self.object.id)
